...
# from qwen_omni_utils import  _fetch_video_from_frames
import glob
...
class MMPluginMixin:
    ...
    def _regularize_videos(self, videos: list["VideoInput"], **kwargs) -> dict[str, list[list["ImageObject"]]]:
        ...
        for video in videos:
            ...
            # frames = get_train_frames(frame_dir="/remote-home1/share/data/longvale/train_extracted_frames", video=video_name)
...
class Qwen2OmniPlugin(Qwen2VLPlugin):
    ...
    def _regularize_audios(
        ...
        for audio in audios:
            if not isinstance(audio, np.ndarray):
                chunks = audio.split("/")
                audio_path = "/".join(chunks[:-2])
                offset = 0
                duration = eval(chunks[-2])
                print(f"audio_path:{audio_path}")
                audio, sampling_rate = librosa.load(audio_path, sr=sampling_rate,offset=offset,duration=duration)
    ...
    def _regularize_videos(
        ...
        for video in videos:
            ...
            if _check_video_is_nested_images(video):
                ...
                # video: xxx/duration_seg/duration
                chunks = video.split("/")
                video_path = "/".join(chunks[:-2])
                duration_seg = eval(chunks[-2])
                duration = eval(chunks[-1])
                container = av.open(video_path, "r")
                video_stream = next(stream for stream in container.streams if stream.type == "video")
                sample_indices = self._get_video_sample_indices(video_stream, **kwargs)
                container.seek(0)
                ratio = duration_seg / duration
                split_point = max(int(ratio * len(sample_indices)), 1)
                                  
                for frame_idx, frame in enumerate(container.decode(video_stream)):
                    if frame_idx in sample_indices[:split_point]:
                        frames.append(frame.to_image())
                ...
                if video_stream.duration is None:
                    ...
                    fps_per_video.append(len(sample_indices) / float(video_stream.duration * video_stream.time_base))
                    fps_per_video.append(split_point / duration_seg)
                    # fps_per_video.append(len(sample_indices) / duration_seg)
    ...
    def _get_mm_inputs(
        ...
        if pt_dir is not None:        
            video_name = str(videos[-1].split("/")[-1]) + ".pt"
            pt_path = os.path.join(pt_dir, video_name)
            ...
            if os.path.exists(pt_path):
                try:
                    ...
                    print(f"loading pt_path:{pt_path} successfully!")
                    
                    return x
                except:
                    print(f"fail loading pt_path:{pt_path}")
        image_processor: BaseImageProcessor = getattr(processor, "image_processor", None)
        feature_extractor: SequenceFeatureExtractor = getattr(processor, "feature_extractor", None)
        mm_inputs = {}
        if len(images) != 0:
        ...
        if len(videos) != 0:
            # pdb.set_trace()
            # max_pixel = getattr(processor, "video_max_pixels", 256 * 256)
            # frs = getattr(processor, "video_maxlen", 128)
            # print(f"image_max_pixels:{max_pixel} video_maxlen:{frs}")
            video_dict = self._regularize_videos(
                videos,
                image_max_pixels=getattr(processor, "video_max_pixels", 256 * 28 * 28),
                image_min_pixels=getattr(processor, "video_min_pixels", 16 * 16),
                video_fps=getattr(processor, "video_fps", 2.0),
    ...
    def process_messages(
        ...
        use_audio_in_video = getattr(processor, "use_audio_in_video", True)
        if self.expand_mm_tokens:
            mm_inputs = self._get_mm_inputs(images, videos, audios, processor, pt_dir=pt_dir)
            image_grid_thw = mm_inputs.get("image_grid_thw", [])
            video_grid_thw = mm_inputs.get("video_grid_thw", [])
            if "feature_attention_mask" in mm_inputs:
        ...
        for message in messages:
            ...
            message["content"] = content
        # print(f"num_image_tokens:{num_image_tokens} num_video_tokens:{num_video_tokens} num_audio_tokens:{num_audio_tokens}")
        return messages
