    ...
    def __call__(
        ...
        **kwargs: Unpack[Qwen2_5OmniProcessorKwargs],
        ...
        if read_pt is None:
            ...
            print(f"Loading preprocessed feature {read_pt}")
            if isinstance(read_pt, list):
                read_pt = read_pt[0]
            
            feature_inputs = torch.load(read_pt)
            if 'feature_attention_mask' in feature_inputs and 'input_features' in feature_inputs:
                audio_inputs = {
                    "feature_attention_mask": feature_inputs['feature_attention_mask'],
            ...
            images_inputs = {}
            image_grid_thw = iter([])
            if 'pixel_values_videos' in feature_inputs:
                videos_inputs = {
                    "pixel_values_videos": feature_inputs["pixel_values_videos"],
                    "video_grid_thw": feature_inputs["video_grid_thw"],
                    "video_second_per_grid": feature_inputs["video_second_per_grid"]
                }
                video_grid_thw = iter(videos_inputs["video_grid_thw"])
                video_second_per_grid = iter(np.array(videos_inputs["video_second_per_grid"]))
            else:
                videos_inputs = {}
                video_grid_thw = iter([])
                video_second_per_grid = iter([])
